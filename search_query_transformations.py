...
if __name__ == '__main__' :
    ...
    finder.search_query_transformations(params, params.query_class, params.query_instance, queries_transformated)
    #get detection results to resume in one folder
    results_query = open('{0}/{1}/{2}_transformations/{3}/detections/{4}/{5}.txt'.format(params.feat_savedir, params.dataset_name, params.model + '_' + params.layer, params.principal_components,  params.query_class, params.query_instance.replace('.png','').replace('.jpg','')),'r')
    
    

    if not os.path.isdir(params.feat_savedir + '/' + params.dataset_name + '/' + params.model + '_transformations/' + str(params.principal_components) + '/detections/'+params.query_class):
        os.makedirs(params.feat_savedir + '/' + params.dataset_name + '/' + params.model + '_transformations/' + str(params.principal_components) + '/detections/'+params.query_class)

    results_transformations = open('{0}/{1}/{2}/{3}/detections/{4}/{5}.txt'.format(params.feat_savedir, params.dataset_name, params.model + '_transformations', params.principal_components,  params.query_class, params.query_instance.replace('.png','').replace('.jpg','')),'w')
    ...
    for line in times_file.readlines():
        ...
        if instance == params.query_instance:
