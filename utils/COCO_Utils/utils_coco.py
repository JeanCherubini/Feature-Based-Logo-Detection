...
class Dataset(object):
    ...
    def load_annotations(self, image_id):
        """Load the specified queries that belong to the imaghe return them as a list"""
    ...
    def load_annotations_batch(self, ids_list):
        ...
        annotations_list = []
        for _id in ids_list:
            annotation = self.load_annotations(_id)
            cant_annot, width = annotation.shape
            if cant_annot>max_cant:
                max_cant = cant_annot
            annotations_list.append(annotation)

        canvas = np.zeros([len(ids_list), max_cant, width])

        for n in range(len(annotations_list)):
            annot = annotations_list[n]
            cant, width = annot.shape
            canvas[n, :cant, :width] = annot
