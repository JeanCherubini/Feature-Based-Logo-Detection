...
if __name__ == '__main__' :        
    ...
    with strategy.scope():
        ...
        model_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(
            filepath=configuration.get_snapshot_dir() + '{epoch:03d}_' + datetime.now().strftime("%Y%m%d-%H%M")+ '.h5',
        ...
        csv_callback = tf.keras.callbacks.CSVLogger(configuration.get_snapshot_dir() + datetime.now().strftime("%Y%m%d-%H%M")+ '.csv', separator=",", append=False)
        ...
        if configuration.get_model_name() == 'UVROIS' :            
            ...
            model = resnet.ResNet([3,4,6,3],[64,128,256,512], configuration.get_number_of_classes(), se_factor = 1)
