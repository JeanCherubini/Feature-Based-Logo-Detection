...
import sys
from models.non_local_tf import NONLocalBlock2D

sys.path.append("/home/jeancherubini/Documents/oneshot_detection")
...
class ResidualBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, filters, stride, use_projection = False, se_factor = 0,  **kwargs):        
        ...
        if se_factor > 0 :
            ...
            print('Using Squeeze And Excitation Block')
...
class ResNet_SE_Nonlocal(tf.keras.Model):
    """ 
    ResNet model with modules of squeeze and excitation and non-local features
    e.g.    
    block_sizes: it is the number of residual components for each block e.g  [2,2,2] for 3 blocks 
    filters : it is the number of channels within each block [32,64,128]
    number_of_classes: The number of classes of the underlying problem
    ...
    def __init__(self, block_sizes, filters, number_of_classes, use_bottleneck = False,  se_factor = 0, **kwargs):
        super(ResNet_SE_Nonlocal, self).__init__(**kwargs)
        self.backbone = ResNetBackbone(block_sizes, filters, use_bottleneck, se_factor, name = 'backbone')                                
        
        self.avg_pool = tf.keras.layers.GlobalAveragePooling2D() 
        
        self.se_module = SEBlock(filters[-1], filters[-1])

        self.non_local_block = NONLocalBlock2D(filters[-1])
        self.non_local_block.build(64)


        print('------------------------------------------------------------------------------------------')
        print('Using Squeeze and Excitation and Non-local features after resnet conv blocks')
        print('------------------------------------------------------------------------------------------')   
