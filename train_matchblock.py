...
import argparse
import os
from utils.metrics import simple_accuracy
...
if __name__ == '__main__' :        
    ...
    parser.add_argument("-opt", type = str,  help = "Optimizer to use", required = True)    
    parser.add_argument("-se_factor", type = int, help = "Factor of squeeze and excitation block on Resnet", required = True, default = 0)    
    parser.add_argument("-non_local", type = bool, help = "Use non local features on Resnet", required = True, default = False)    
    ...
    #Load tfrecors for training, test in sigle or multicore 
    if pargs.mode == 'train' :
    ...
    with strategy.scope():
        ...
        model_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(
            filepath=configuration.get_snapshot_dir() + configuration.get_model_name() +'_'+ '{epoch:03d}_' + datetime.now().strftime("%Y%m%d-%H%M")+ '.h5',
        ...
        #Callback for saving metrics to csv
        csv_callback = tf.keras.callbacks.CSVLogger(configuration.get_snapshot_dir() + configuration.get_model_name() +'_'+ datetime.now().strftime("%Y-%m-%d-%H-%M")+ '.csv', separator=",", append=False)

        #resnet 34        
        if pargs.non_local:
            model = resnet.ResNet_SE_Nonlocal([3,4,6,3],[64,128,256,512], configuration.get_number_of_classes(), se_factor = pargs.se_factor)
            print('Model is Resnet')
            sys.stdout.flush()
        else:
            model = resnet.ResNet([3,4,6,3],[64,128,256,512], configuration.get_number_of_classes(), se_factor = pargs.se_factor)
            print('Model is Resnet')
            sys.stdout.flush()
        #resnet_50
        ...
        if pargs.mode == 'train' :                             
            history = model.fit(tr_dataset, 
                            ...
                            callbacks=[model_checkpoint_callback, csv_callback, tensorboard_callback])
